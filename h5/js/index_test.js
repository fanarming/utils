function Ship(ctx){
	...
	this.player = gameMonitor.im.createImage('./img/player.png');
...
console.log(this.left);
	...
	this.controll = function(){
		...
		$(window).on(gameMonitor.eventType.start, function(event){
			_this.setPosition(event);
			move = true;
		}).on(gameMonitor.eventType.end, function(){
			move = false;
...
var gameMonitor = {
	...
	initListener : function(ctx){
        ...
        /*设置键盘左右移动*/
		body.on('click', '#guidePanel', function(){
			$(this).hide();
			var canvas = document.getElementById('stage');
			var ctx = canvas.getContext('2d');
			_this.ship = new Ship(ctx);
...
if(!gameMonitor.isMobile()){
	...
	gameMonitor.eventType.start = 'keydown';
	gameMonitor.eventType.move = 'keypress';
	gameMonitor.eventType.end = 'keyup';
}
